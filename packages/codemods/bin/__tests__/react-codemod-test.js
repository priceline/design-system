...
const { runTransform, jscodeshiftExecutable, transformerDirectory } = require('../cli')
...
describe('check-git-status', () => {
  ...
  it('does not exit when git repo is dirty and force flag is given', () => {
    ...
    process.exit = jest.fn()
    checkGitStatus(true)
    expect(console.log).toBeCalledWith('WARNING: Git directory is not clean. Forcibly continuing.')
    expect(process.exit).not.toBeCalled()
  })
})
