...
export const mapProps = (map) => (Component) => hoistStatics((props) => React.createElement(Component, Object.assign({}, map(props))), Component);
// Use this to mark props as deprecated
export const deprecatedPropType = (replacement) => (props, propName) => {
    if (props[propName]) {
        return new Error(`The \`${propName}\` prop is deprecated and will be removed in a future release. Please use \`${replacement}\` instead.`);
    }
};
export const deprecatedColorValue = () => (props, propName, componentName) => {
    if (process.env.NODE_ENV !== 'production' &&
        props.theme &&
        props[propName] &&
        !hasPaletteColor(Object.assign({ color: props[propName] }, props))) {
        return new Error(`The color value of \`${props[propName]}\` for \`${componentName}\` is deprecated and will be removed in a future release. Please use a palette color instead.`);
    }
};
/**
 ...
 * @returns {array}
 */
export const applySizes = (sizes = null, defaultSize = 'medium') => ({ size, }) => {
    return (sizes &&
        typeof size === 'string' &&
        css `
