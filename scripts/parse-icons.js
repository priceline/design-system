...
const readIcons = dir => fs.readdirSync(dir)
...
const getPath = nodes => nodes.children
...
const removePrefix = str => str.replace(/^ic\-/, '') // eslint-disable-line no-useless-escape

const getData = icons => icons.map(icon => Object.assign({}, icon, parseSVG(icon.svg)))

const build = () => {
  const icons = readIcons(src)
  const data = getData(icons)
    .reduce((a, { key, svg, viewBox, path }) => Object.assign({}, a, {
      [key]: {
        viewBox,
        path
      }
